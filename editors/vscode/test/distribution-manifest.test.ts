...
const fixtureVersion = JSON.parse(fixture).packageVersion;

test("公開manifestからplatformに一致するassetを一意に選択します", () => {
  const manifest = parseDistributionManifest(fixture, fixtureVersion);
...
test("未知field、version不一致、重複assetを拒否します", () => {
  ...
  assert.throws(
    () => parseDistributionManifest(JSON.stringify({ ...parsed, unknown: true }), fixtureVersion),
    /invalid-manifest/,
  );
