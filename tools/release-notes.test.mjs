...
test(`Release Notesはv${RELEASE_NOTES_VERSION}の変更内容と移行方法を含む`, () => {
  ...
  assert.equal(PREVIOUS_RELEASE_VERSION, "0.32.0");
  assert.match(notes, /#476：/);
  assert.match(notes, /AsciiDocへ文字列連結せず/);
  assert.match(notes, /RustとWASMで同じ契約/);
  ...
  assert.match(notes, /v0\.32\.0の9から更新/);
  assert.match(notes, /Worker protocol versionは2のまま/);
  assert.match(notes, /schema versionは4のままで、項目を追加も削除もしていません/);
  assert.match(notes, /Rust APIの破壊的変更/);
  assert.match(notes, /``WasmRenderInputs``へ/);
  assert.match(notes, /``generated_bibliography: None``を追加/);
